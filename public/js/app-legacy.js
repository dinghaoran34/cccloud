  ...
  function handleLegacySubmit(e) {
    ...
    xhr.onreadystatechange = function() {
      ...
      if (xhr.status >= 200 && xhr.status < 400) {
        ...
        if (etaEl) etaEl.innerHTML = '[成功] 上传完成 ' + file.name;
        if (submitBtn) submitBtn.innerHTML = '[成功] 上传完成!';
        ...
        if (etaEl) etaEl.innerHTML = '[失败] 上传失败: HTTP ' + xhr.status;
        if (submitBtn) {
          submitBtn.disabled = false;
          submitBtn.innerHTML = '[失败] 上传失败, 重试';
        }
      }
    };
